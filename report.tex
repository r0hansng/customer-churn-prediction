...
The retention engine is surfaced in the Single Prediction UI. After a high-risk prediction is confirmed by the ensemble, a ``Generate Retention Strategy (AI)'' button appears. Clicking it invokes \texttt{run\_retention\_engine()} and renders the Gemini-generated strategy inline with Markdown formatting. API keys are retrieved securely from Streamlit Secrets on Streamlit Cloud, with a local \texttt{.env} fallback for development.
...
\begin{itemize}[leftmargin=2em]
    ...
    \item A \textbf{LangGraph + RAG retention engine} (Gemini 2.5 Flash + FAISS) generates personalised, policy-grounded retention plans for flagged customers.
    \item A \textbf{fully integrated Streamlit dashboard} provides single-customer inference with ensemble verdict and AI strategy, bulk batch processing, and a metrics comparison page.
